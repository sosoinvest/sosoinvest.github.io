...
샤프지수는 세금이 없는 경우 0.68, 세금이 있는 경우 0.74로 세금이 있는 경우가 조금 더 높았다. 그 이유는 수익률은 적지만 마찬가지로 변동성 또한 작기 때문이다. Buy and hold가 0.29로 가장 안좋았고, S&P 500은 0.87로 가장 좋았다. S&P 500 지수가 변동성 돌파전략보다 수익률은 더 낮지만, 변동성이 그보다 더 작기 때문일 것이다.

여기서 샤프지수의 계산에는 일간 기하 평균 수익률과 일간 수익률의 표준편차를 이용했다. 즉, 

$$
샤프지수=\frac{일간~기하평균~수익률}{일간~수익률의~표준편차}
$$

여기서, 

$$
일간~기하평균~수익률=(\frac{최종 자산}{초기 자산})^{1/n}
$$

n은 총 거래일수를 의미한다. 전문가들은 샤프지수 계산에 무위험 수익률을 넣어서 계산하지만 여기서는 계산의 편의성을 위해 무위험 수익률을 0으로 보았다. 
...
샤프지수는 K값이 0.1에서 가장 작고, K와 함께 증가하여 0.9일 때 가장 높았다. 아무래도 K값이 커질수록 보수적으로 매매하기 때문에 변동성이 줄어들기 때문으로 이해된다. 한가지 주목해야할 것은 K가 0.3일 때보다 0.7일 때가 샤프지수와 수익률 모두 높은 것으로 보아서, K=0.3 보다 K=0.7이 변동성도 적고 수익도 높은 우수한 값이라는 점이다.
...
| 샤프지수  | 0.46 | 0.54 | 0.68 | 0.83 | 1.04 |

# 죽음과 세금은 피할 수 없다.
앞에 결과에서 봤듯이 세금의 유무에 따라 전략의 결과가 천지차이다. 이러한 결과의 기저에는 세금으로 인해 복리의 마법이 발목을 잡히는 이유가 있다.
