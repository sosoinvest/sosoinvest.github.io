...
동전에 대한 정보가 전혀 없을 떄, 사전 확률 밀도 함수가 $[0,1]$ 구간에서 균일(uniform)하다고 가정해보자. 즉, $g(r)=1$. 실제 우리가 사용하는 동전은 앞면이 나올 확률이 50%에 가까울 것이기 때문에, 확률 밀도 함수는 $r=0.5$에 가중된 모양일 것이다. 그런데, 동전이 전혀 정보가 없는 순수한 블랙박스라면, 확률 밀도 함수가 균일하다는 가정에서 출발해서 동전던지기에 새로 얻는 정보를 이용하여 베이즈 이론으로 계속해서 업데이트 해나갈 수 있다. 
...
$$
여기서 h와 t는 정수이기 떄문에, 위의 사후 확률 밀도식은 팩토리얼을 이용해 다음과 같이 표현된다.
...
r이 45%에서 55% 사이에 있을 떄 동전이 공전하다고 판정한다면, 시행결과를 바탕으로 추정한 이 범위에 들어가 확률은, 
...
### 예제
1. 최대 허용오차가 0.01이라고 할 떄, 동전을 몇번 던져야 하는가?

위의 수식에 $E=0.01$을 대입하면, $n=2500Z^2$.

\- 신뢰도 68.27%(Z=1) 일 떄, n=2500,
...
추정 확률 r과 최대오차 E일 떄, 실제 확률 r의 범위는,
$p-E<r<p+E$ 따라서,
